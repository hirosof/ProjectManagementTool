...
### v2（2026-01-16）
- **重要**: 親子関係のFKを `ON DELETE RESTRICT` に変更（D6「子持ち削除デフォルト禁止」に準拠）
- 依存関係のFKは `ON DELETE CASCADE` を維持（削除時の自動クリーンアップ）
...
**FK削除動作**:
- `ON DELETE RESTRICT`: 子SubProject/Taskが存在する場合、Project削除は拒否される
...
with db.transaction():
    ...
    # 2. ステータス更新
    update_status(node_id, new_status)
```
...
2. **order_index**: 0始まり
3. **タイムスタンプ**: UTC固定（`datetime('now')`）
4. **論理削除**: MVPでは実装しない
5. **テンプレートテーブル**: Phase 3 で追加
6. **削除制御**: アプリケーション層で子チェック + 橋渡し処理
...
本DB設計書（v2）を確認いただき、問題なければ以下に進みます:
